...
php tools/backfill-pose-numbers.php --dry-run   # Assign ld_artworks.pose_number to pre-migration-022 artwork
php tools/strip-jpeg-trailers.php --dir=storage/photo-import/ID          # Report data hidden past a JPEG's EOI marker
php tools/strip-jpeg-trailers.php --dir=storage/photo-import/ID --strip  # Remove it (lossless; verified before replacing)
php tools/audit-public-trailers.php --session=ID   # Assert nothing in the public tree carries a trailer (run on prod)
php tools/test-jpeg-trailer.php        # Test the JPEG marker walk (no DB; --corpus=PATH to re-walk staged photos)
...
2.5. **Strip JPEG trailers** — `tools/strip-jpeg-trailers.php --dir=storage/photo-import/{id} --strip`. Samsung hides payloads after the JPEG's end-of-image marker: Motion Photo appends a short video *with room audio*, Live Focus appends a complete second photograph of the room. Both pass `finfo` and `getimagesize`, and originals are served statically from `public/assets/uploads/`, so nothing downstream would have caught them. Truncating at EOI is lossless — dimensions, EXIF and decoded pixels are verified identical before any original is replaced. Every trailer is removed, not just the ones classified as carriers, so "no bytes past EOI" is a post-condition rather than a heuristic's opinion. Run this **before** the `_backup` copy so both sides stay byte-identical, and never retroactively on an existing `_backup/` — the Instagram posted-ledger is keyed on `sha1_file()` of those exact bytes. `import-session-photos.php` strips as a backstop regardless, recording what it removed in provenance.
3. **Transfer + import** — `scp` the staged folders to the server, then run `import-session-photos.php` per directory, in pose order so `pose_index` stays chronological. One directory is one pose: the importer stamps a shared `pose_number` (migration 022) on every row it writes, which is what keeps two poses of the same length from reading as a single block on the session page. The importer validates real MIME type + image integrity, leaves WebP derivatives NULL for `process_images.php` to generate, writes `artwork.upload` provenance, and is rerun-safe (dedup by original filename + content hash) and orphan-safe (copy-then-insert, rollback on failure). Compare a SHA1 rollup of both sides after the transfer; byte totals alone differ harmlessly because `du` counts directory entries.
